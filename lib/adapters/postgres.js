...
PG.prototype.updateWithRow = function(model, filter, data, callback) {
    if ('function' === typeof filter) {
        return filter(new Error("Get parametrs undefined"), null);
    }
    if ('function' === typeof data) {
...
function parseCond(cs, key, props, conds, self) {
    ...
    if (conds[key] === null) {
        ...
        Object.keys(conds[key]).forEach(function(condType) {
            if ((condType === 'inq' || condType === 'nin' || condType === 'in') && conds[key][condType]) {
              val = '';
              conds[key][condType].forEach(function(item) {
                val += item + ',';
              });
