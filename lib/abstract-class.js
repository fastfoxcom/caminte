...
AbstractClass.updateWithRow = function updateWithRow(filter, data, options, callback) {
    if (stillConnecting(this.schema, this, arguments)) {
        return;
    }

    if (this.schema.adapter.updateWithRow) {
        this.schema.adapter.updateWithRow(this.modelName, filter, data, options, function (err, affected) {
            callback(err, affected);
        }.bind(this));
    } else {
        console.log('Method update undefined for this adapter');
